...
void makeMuDstQA_local(TString InputFileList, Int_t nFiles = 1, Int_t nEvents = 0, TString OutputDir = "output/" );

void makeMuDstQA_local(TString InputFileList, Int_t nFiles, Int_t nEvents, TString OutputDir ) 
{
  ...
  if ( nEvents == 0 )  nEvents = 1000 ;       // Take all events in nFiles if nEvents = 0
...
if (indx1 >= 0 && indx2 >= 0 && indx1 != indx2) {
  ...
  // ---------------- modify here according to your QA purpose --------------------------
     ...
     //vzVpd
     StBTofHeader *mBTofHeader = (StBTofHeader*)mMuDst->btofHeader();
     Float_t vzVpd=-999;
     if (mBTofHeader) vzVpd = mBTofHeader->vpdVz();

     //-----------------------------------------------------------------------------
// ROOT5/CINT-friendly replacement of PicoVtxMode enum
...
const double mTpcVpdVzDiffCut = 3.0;
	...
	Bool_t isRun14MB = kFALSE;

	isRun14MB =
	  mMuEvent->triggerIdCollection().nominal().isTrigger(450005) ||
	  mMuEvent->triggerIdCollection().nominal().isTrigger(450015) ||
	  mMuEvent->triggerIdCollection().nominal().isTrigger(450025) ||
	  mMuEvent->triggerIdCollection().nominal().isTrigger(450050) ||
	  mMuEvent->triggerIdCollection().nominal().isTrigger(450060);

	if (!isRun14MB) continue;
     //Vz
