...
void makeMuDstQA_Run14(TString InputFileList, Int_t nFiles, Int_t nEvents, TString OutputDir ) 
{
...
#ifdef __CINT__
  gROOT->Macro("/gpfs/mnt/gpfs01/star/pwg/lomicond/Ondrej/Jets/PythiaD0JetGeant/D0EmbeddingClean/loadMuDstLibraries_SL16d.C");
#endif
  ...
  // ---------------- modify here according to your QA purpose --------------------------
     ...
     //trigger
// Run14 Au+Au 200 GeV MB triggers
if ( ! mMuEvent->triggerIdCollection().nominal().isTrigger(450050) &&
     ...
     ! mMuEvent->triggerIdCollection().nominal().isTrigger(450025) ) continue ;
     //Vz
