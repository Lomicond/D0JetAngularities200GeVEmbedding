  ...
  const char* ParticleNameFromPdg(Int_t pdg)
{
    ...
    // Neutrinos are intentionally not transported.
    if (absPdg == 12 || absPdg == 14 || absPdg == 16) {
        return "";
    }

    TParticlePDG* particle =
        StarParticleData::instance().GetParticle(pdg);

    if (!particle) {
        std::cout << "WARNING: final-state PDG " << pdg
                  << " was not found in StarParticleData and will be skipped"
                  << std::endl;
        return "";
    }
