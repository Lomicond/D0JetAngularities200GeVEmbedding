...
const StMuMcTrack* checkedMcTrackFromTruthId(Int_t idTruth)
{
    TClonesArray* mcTracks = StMuDst::mcArray(MCTrack);

    if (!mcTracks) {
        return 0;
    }

    const Int_t nMcTracks = mcTracks->GetEntriesFast();

    if (idTruth <= 0 || idTruth > nMcTracks) {
        return 0;
    }

    const StMuMcTrack* mcTrack =
        static_cast<const StMuMcTrack*>(
            mcTracks->UncheckedAt(idTruth - 1)
        );
    ...
    // Same consistency check used by STAR embedding QA examples.
    if (mcTrack->Id() != idTruth) {
        return 0;
    }
...
bool keepMcRecoTrack(const StMuTrack* track)
{
    if (!track) {
        return false;
    }

    if (!checkedMcTrackFromTruthId(track->idTruth())) {
        return false;
    }

    // STAR embedding QA working point.
    if (track->qaTruth() < 50) {
        return false;
    }
...
bool keepMcBTofHit(const StMuBTofHit* hit)
{
    if (!hit) {
        return false;
    }

    // In the mixed MuDst, simulated BTOF hits carry qaTruth == 1.
    // Real/background hits have qaTruth == 0.
    if (hit->qaTruth() != 1) {
        return false;
    }
...
void StPicoDstMaker::fillTracks() {
  ...
  for (int i = 0; i < nGlobals; ++i) {
    ...
    */
    ...
    // TOF information
    const StMuBTofHit* tofHit = gTrk->tofHit();
