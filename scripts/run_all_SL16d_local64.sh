...
# =============================================================================
# D0EmbeddingClean end-to-end runner - SL16d_embed2 local64 variant
#
# Stages:
#   1. Standalone Pythia8 generation (custom Pythia 8.317)
#   2. Run14 MoreTags event selection (STAR SL16d_embed2 + local64 libs)
#   3. DAQ chopping to the first N selected events (STAR SL16d_embed2 + local64 libs)
#   4. STARSIM / GEANT3 (STAR SL16d_embed2 + local64 libs)
#   5. Mixer / reconstruction (STAR SL16d_embed2 + local64 libs)
#   6. PicoDst production from embedding MuDst (STAR SL16d_embed2 + local64 libs)
#
# The script is intended to live at:
#   scripts/run_all_SL16d_local64.sh
#
# Example:
#   ./scripts/run_all_SL16d_local64.sh \
#       --nevents 200 \
...
STAR_LEVEL="SL16d_embed2"

# Local 64-bit compatibility layer for SL16d_embed2.
# This directory is expected to contain .${STAR_HOST_SYS}/lib with the locally
# built SL16d_embed2 libraries, e.g. libStChain.so, libStBFChain.so,
# libStEmcRawMaker.so, libStEmcMixerMaker.so, libStEvent.so, libStarRoot.so.
SL16D_LOCAL64="${SL16D_LOCAL64:-/gpfs/mnt/gpfs01/star/pwg/lomicond/Ondrej/Jets/PythiaD0JetGeant/SL16d_embed2_local64_full}"
LOCAL_ROOT4STAR_BIN="${ROOT_DIR}/local_sl16d_bin"
...
usage() {
    cat <<'USAGE'
Usage:
  ./scripts/run_all_SL16d_local64.sh --nevents N [options]
...
STAR:
  --star-level LEVEL
      STAR release. Default: SL16d_embed2
  --sl16d-local64 DIR
      Local 64-bit compatibility build for SL16d_embed2.
      Default:
      /gpfs/mnt/gpfs01/star/pwg/lomicond/Ondrej/Jets/PythiaD0JetGeant/SL16d_embed2_local64_full
...
while (($#)); do
    case "$1" in
        ...
        --star-level)
            ...
            ;;
        --sl16d-local64)
            [[ $# -ge 2 ]] || die "--sl16d-local64 requires a value"
            SL16D_LOCAL64="$2"
            shift 2
            ;;
...
SL16D_LOCAL64="$(resolve_existing_dir "$SL16D_LOCAL64")" || die "SL16d local64 directory not found: $SL16D_LOCAL64"
...
# -----------------------------------------------------------------------------
# ROOT load block for the local SL16d 64-bit compatibility libraries.
# Keep this order.  It was tested explicitly in root4star.
# -----------------------------------------------------------------------------
ROOT_LOAD_SL16D_LOCAL64="$(cat <<'ROOTLOAD'
...
# Prefer the local SL16d_embed2 64-bit compatibility libraries.
setenv D0WF_SL16D_LOCAL64 "${SL16D_LOCAL64}"
set local_sl16d_lib = "${SL16D_LOCAL64}/.\$STAR_HOST_SYS/lib"
...
# The official SL16d_embed2 64-bit release can lack root4star/bin content.
# Put the local root4star wrapper first, when available.
if ( -d "${LOCAL_ROOT4STAR_BIN}" ) then
    setenv PATH "${LOCAL_ROOT4STAR_BIN}:\${ROOTSYS}/bin:\${PATH}"
else
    setenv PATH "\${ROOTSYS}/bin:\${PATH}"
endif

if ( \$?LD_LIBRARY_PATH ) then
    setenv LD_LIBRARY_PATH "$D0DECAY_OVERLAY/lib:\${local_sl16d_lib}:\${STAR}/.\${STAR_HOST_SYS}/lib:\${ROOTSYS}/lib:\${LD_LIBRARY_PATH}"
else
    setenv LD_LIBRARY_PATH "$D0DECAY_OVERLAY/lib:\${local_sl16d_lib}:\${STAR}/.\${STAR_HOST_SYS}/lib:\${ROOTSYS}/lib"
endif
...
${ROOT_LOAD_SL16D_LOCAL64}

.x ${ROOT_DIR}/src/moretags/makeMuDstQA_Run14.C("${JOB_MUDST_LIST}",1,${MORETAGS_SCAN_EVENTS},"${MORETAGS_DIR}/")
...
if ( \$rc != 0 ) then
    if ( -s "" && -s "" ) then
        echo "WARNING: MoreTags root4star exited with status \$rc after writing outputs; treating as OK"
        set rc = 0
    endif
endif
exit \$rc
...
.L /gpfs/mnt/gpfs01/star/pwg/lomicond/Ondrej/Jets/PythiaD0JetGeant/D0EmbeddingClean/src/starsim/starsim.D0toy.C
...
# Optional project-local STAR libraries, e.g. a locally built StPicoDstMaker.
# In this SL16d variant the mandatory local SL16d libraries are already added
# by write_star_prelude().  Do not fail if this project-local directory is absent.
set local_star_lib = "${ROOT_DIR}/.\$STAR_HOST_SYS/lib"

if ( -d "\$local_star_lib" ) then
    if ( \$?LD_LIBRARY_PATH ) then
        setenv LD_LIBRARY_PATH "$D0DECAY_OVERLAY/lib:\${local_star_lib}:\${LD_LIBRARY_PATH}"
    else
        setenv LD_LIBRARY_PATH "$D0DECAY_OVERLAY/lib:\${local_star_lib}"
    endif
    echo "D0WF_LOCAL_STAR_LIB=\$local_star_lib"
else
    echo "D0WF_LOCAL_STAR_LIB=missing_optional:\$local_star_lib"
endif
