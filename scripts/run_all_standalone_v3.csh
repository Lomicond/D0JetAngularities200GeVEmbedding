...
# Default test:
#
#   tcsh scripts/run_all_standalone_v2.csh
#
# Arguments:
#
...
#
#   Fresh 5-event run:
#       tcsh scripts/run_all_standalone_v2.csh 5 1 0
#
#   Resume from STARSIM:
#       tcsh scripts/run_all_standalone_v2.csh 5 4 0
#
#   Force only PicoDst:
#       tcsh scripts/run_all_standalone_v2.csh 5 6 1
#
...
# ============================================================================
# Reference jobs used as immutable templates
# ============================================================================

set BASE_SRCJOB = \
"$D0WF/work/jobs/st_physics_15130045_raw_1000011_200evts_droyMatch8303_v1"

set BASE_STANDJOB = \
"$D0WF/work/jobs/st_physics_15130045_raw_1000011_200evts_standalone_v1"

if ( ! -d "$BASE_SRCJOB" ) then
    echo "ERROR: reference source job is missing:"
    echo "       $BASE_SRCJOB"
    exit 7
endif

if ( ! -d "$BASE_STANDJOB" ) then
    echo "ERROR: reference standalone job is missing:"
    echo "       $BASE_STANDJOB"
    exit 8
endif
...
set SRCJOB = \
"$D0WF/work/jobs/st_physics_15130045_raw_1000011_${NEVENTS}evts_droyMatch8303_v1"

set STANDJOB = \
"$D0WF/work/jobs/st_physics_15130045_raw_1000011_${NEVENTS}evts_standalone_v1"

set SRCDRIVERS = "$SRCJOB/drivers"
set STANDDRIVERS = "$STANDJOB/drivers"

set PYTHIADIR = "$SRCJOB/pythia"
set MORETAGSDIR = "$SRCJOB/moretags"
set STARSIMDIR = "$SRCJOB/starsim"
...
set LOGDIR = \
"$STANDJOB/logs/run_all_standalone_v3_$RUN_TAG"
...
set BASE_MUDST_LIST = \
"$BASE_SRCJOB/mudst.selected.list"

set MUDST_LIST = \
"$SRCJOB/mudst.selected.list"

if ( ! -s "$BASE_MUDST_LIST" ) then
    echo "ERROR: reference MuDst list is missing:"
    echo "       $BASE_MUDST_LIST"
    exit 10
endif

cp -p "$BASE_MUDST_LIST" "$MUDST_LIST"

set BASE_PYTHIA_SOURCE = \
"$BASE_SRCJOB/pythia/make_d0_pythia8_used.cc"

set BASE_PYTHIA_CONFIG = \
"$BASE_SRCJOB/pythia/droy_hybrid_used.cmnd"

set PYTHIA_SOURCE = \
"$PYTHIADIR/make_d0_pythia8_used.cc"

set PYTHIA_CONFIG = \
"$PYTHIADIR/droy_hybrid_used.cmnd"

if ( ! -s "$BASE_PYTHIA_SOURCE" ) then
    echo "ERROR: frozen PYTHIA source is missing:"
    echo "       $BASE_PYTHIA_SOURCE"
    exit 11
endif

if ( ! -s "$BASE_PYTHIA_CONFIG" ) then
    echo "ERROR: frozen PYTHIA configuration is missing:"
    echo "       $BASE_PYTHIA_CONFIG"
    exit 12
endif

cp -p "$BASE_PYTHIA_SOURCE" "$PYTHIA_SOURCE"
cp -p "$BASE_PYTHIA_CONFIG" "$PYTHIA_CONFIG"

# ============================================================================
# Build event-count-specific drivers from validated templates
#
# Important:
# Some reference drivers contain literal "$D0WF/work/jobs/..." paths
# rather than fully expanded absolute paths. Therefore replace the job
# directory NAMES, not complete absolute paths.
# ============================================================================

set BASE_SRC_NAME = `basename "$BASE_SRCJOB"`
set BASE_STAND_NAME = `basename "$BASE_STANDJOB"`

set SRC_NAME = `basename "$SRCJOB"`
set STAND_NAME = `basename "$STANDJOB"`

set STAGE02_DRIVER = \
"$SRCDRIVERS/02_moretags.csh"

set STAGE04_DRIVER = \
"$SRCDRIVERS/04_starsim.csh"

set STAGE05_DRIVER = \
"$STANDDRIVERS/05_standalone_reco_v1.csh"

set STAGE06_DRIVER = \
"$STANDDRIVERS/06_pico_standalone_v1.csh"

sed \
    -e "s|$BASE_SRC_NAME|$SRC_NAME|g" \
    "$BASE_SRCJOB/drivers/02_moretags.csh" \
    >! "$STAGE02_DRIVER"

sed \
    -e "s|$BASE_SRC_NAME|$SRC_NAME|g" \
    -e "s|    200,|    $NEVENTS,|g" \
    "$BASE_SRCJOB/drivers/04_starsim.csh" \
    >! "$STAGE04_DRIVER"

sed \
    -e "s|$BASE_SRC_NAME|$SRC_NAME|g" \
    -e "s|$BASE_STAND_NAME|$STAND_NAME|g" \
    -e "s|set NEVENTS = 200|set NEVENTS = $NEVENTS|g" \
    "$BASE_STANDJOB/drivers/05_standalone_reco_v1.csh" \
    >! "$STAGE05_DRIVER"

sed \
    -e "s|$BASE_STAND_NAME|$STAND_NAME|g" \
    -e "s|    200,|    $NEVENTS,|g" \
    "$BASE_STANDJOB/drivers/06_pico_standalone_v1.csh" \
    >! "$STAGE06_DRIVER"
...
grep -q "$SRC_NAME" "$STAGE02_DRIVER"

if ( $status != 0 ) then
    echo "ERROR: Stage 2 driver does not reference the new source job"
    exit 13
endif

grep -q "$SRC_NAME" "$STAGE04_DRIVER"

if ( $status != 0 ) then
    echo "ERROR: Stage 4 driver does not reference the new source job"
    exit 14
endif

grep -q "$SRC_NAME" "$STAGE05_DRIVER"

if ( $status != 0 ) then
    echo "ERROR: Stage 5 driver does not reference the new source job"
    exit 15
endif

grep -q "$STAND_NAME" "$STAGE05_DRIVER"

if ( $status != 0 ) then
    echo "ERROR: Stage 5 driver does not reference the new standalone job"
    exit 16
endif
...
grep -q "$STAND_NAME" "$STAGE06_DRIVER"

if ( $status != 0 ) then
    echo "ERROR: Stage 6 driver does not reference the new standalone job"
    exit 18
endif

grep -q "$BASE_SRC_NAME" "$STAGE05_DRIVER"

if ( $status == 0 ) then
    echo "ERROR: Stage 5 driver still references the base source job:"
    echo "       $BASE_SRC_NAME"
    exit 19
endif

grep -q "$BASE_STAND_NAME" "$STAGE05_DRIVER"

if ( $status == 0 ) then
    echo "ERROR: Stage 5 driver still references the base standalone job:"
    echo "       $BASE_STAND_NAME"
    exit 20
endif
...
echo
grep -n -E \
'^set SRCJOB|^set WORK|^set NEVENTS' \
"$STAGE05_DRIVER"
echo
...
set PYTHIA_EXE = \
"$PYTHIADIR/make_d0_pythia8_8303"

set PYTHIA_OUTPUT = \
"$PYTHIADIR/pythia8_D0_DetroitTune.root"
...
if ( $START_STAGE <= 1 ) then
    ...
    if ( $STAGE01_VALID == 1 ) then
        ...
        set P8SRC = \
"$D0WF/forensics/droy_pythia_settings_v1/StRoot/StarGenerator/Pythia8_3_03"
