#!/bin/tcsh -f
#
# run_standalone_production_synthetic_moretags_droy64_v1.csh
#
# Standalone D0 MC chain using the prepared synthetic MoreTags file:
#
#   1 PYTHIA 8.303
#   2 Synthetic MoreTags installation and validation
...
# Default test:
#
#   tcsh scripts/run_standalone_production_synthetic_moretags_droy64_v1.csh
#
# Arguments:
#
...
#       tcsh scripts/run_standalone_production_synthetic_moretags_droy64_v1.csh 5 1 0
#
#   Force a complete fresh 5-event rerun:
#       tcsh scripts/run_standalone_production_synthetic_moretags_droy64_v1.csh 5 1 1
#
#   Resume from STARSIM:
#       tcsh scripts/run_standalone_production_synthetic_moretags_droy64_v1.csh 5 4 0
#
#   Force only PicoDst:
#       tcsh scripts/run_standalone_production_synthetic_moretags_droy64_v1.csh 5 6 1
#
...
set SYNTHETIC_MORETAGS_SOURCE = \
"$D0WF/work/moretags_test/synthetic.moretags.root"
...
set SIMJOB = \
"$D0WF/work/jobs/st_physics_15130045_raw_1000011_${NEVENTS}evts_simulation_synthetic_moretags_v1"

set STANDJOB = \
"$D0WF/work/jobs/st_physics_15130045_raw_1000011_${NEVENTS}evts_standalone_synthetic_moretags_droy64_v1"
...
set LOGDIR = \
"$STANDJOB/logs/run_standalone_production_synthetic_moretags_droy64_v1_$RUN_TAG"
...
# ============================================================================
# Stage 2: Synthetic MoreTags installation and validation
# ============================================================================
...
if ( $START_STAGE <= 2 ) then
    ...
    if ( $STAGE02_VALID == 1 ) then
        echo "SKIP: valid synthetic MoreTags input already installed:"
        echo "      $MORETAGS_OUTPUT"
    else
        if ( ! -s "$SYNTHETIC_MORETAGS_SOURCE" ) then
            echo "ERROR: synthetic MoreTags source is missing:"
            echo "       $SYNTHETIC_MORETAGS_SOURCE"
            exit 31
        endif

        root4star -l -b -q \
        'checkRootTree_v1.C("'"$SYNTHETIC_MORETAGS_SOURCE"'","","",1)' \
        >&! "$LOGDIR/02_validate_synthetic_moretags_source.log"

        if ( $status != 0 ) then
            echo "ERROR: synthetic MoreTags source is invalid or empty:"
            echo "       $SYNTHETIC_MORETAGS_SOURCE"
            echo
            cat "$LOGDIR/02_validate_synthetic_moretags_source.log"
            exit 32
        endif

        rm -f "$MORETAGS_OUTPUT"
        cp -p "$SYNTHETIC_MORETAGS_SOURCE" "$MORETAGS_OUTPUT"

        if ( $status != 0 || ! -s "$MORETAGS_OUTPUT" ) then
            echo "ERROR: failed to install synthetic MoreTags input:"
            echo "       source: $SYNTHETIC_MORETAGS_SOURCE"
            echo "       target: $MORETAGS_OUTPUT"
            exit 33
        endif

        root4star -l -b -q \
        'checkRootTree_v1.C("'"$MORETAGS_OUTPUT"'","","",1)' \
        >&! "$LOGDIR/02_validate_installed_synthetic_moretags.log"

        if ( $status != 0 ) then
            echo "ERROR: installed synthetic MoreTags input failed validation:"
            echo "       $MORETAGS_OUTPUT"
            echo
            cat "$LOGDIR/02_validate_installed_synthetic_moretags.log"
            exit 34
        endif

        echo "Synthetic MoreTags installed:"
        echo "      source: $SYNTHETIC_MORETAGS_SOURCE"
        echo "      target: $MORETAGS_OUTPUT"
...
echo "============================================================"
echo "Standalone production with synthetic MoreTags completed"
echo "============================================================"
echo "Events:"
echo "    $NEVENTS"
echo
