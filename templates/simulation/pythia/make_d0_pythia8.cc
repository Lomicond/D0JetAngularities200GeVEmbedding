...
int main(int argc, char* argv[])
{
    ...
    const std::string outputFile = (argc > 4) ? argv[4] : "pythia8_D0_events.root";
    ...
    // Match the reference particle-level convention: keep particles that
    // should be transported and, where applicable, decayed by GEANT stable
    // in the PYTHIA event record. Settings for a positive PDG ID also apply
    // to the corresponding antiparticle.
    pythia.readString("111:onMode = 0");   // pi0
    pythia.readString("211:onMode = 0");   // pi+/-
    pythia.readString("221:onMode = 0");   // eta
    pythia.readString("321:onMode = 0");   // K+/-
    pythia.readString("310:onMode = 0");   // K0S
    pythia.readString("130:onMode = 0");   // K0L
    pythia.readString("3122:onMode = 0");  // Lambda0 / anti-Lambda0
    pythia.readString("3112:onMode = 0");  // Sigma- / anti-Sigma+
    pythia.readString("3222:onMode = 0");  // Sigma+ / anti-Sigma-
    pythia.readString("3212:onMode = 0");  // Sigma0 / anti-Sigma0
    pythia.readString("3312:onMode = 0");  // Xi- / anti-Xi+
    pythia.readString("3322:onMode = 0");  // Xi0 / anti-Xi0
    pythia.readString("3334:onMode = 0");  // Omega- / anti-Omega+
