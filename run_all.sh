...
# =============================================================================
# D0EmbeddingClean end-to-end runner
#
# Stages:
...
#
# Stage names:
#   pythia, moretags, chop, starsim, mixer
# =============================================================================
...
# Stage registry
# -----------------------------------------------------------------------------
STAGE_NAMES=(pythia moretags chop starsim mixer)
STAGE_LABELS=(
    "Pythia8 generation"
    "MoreTags selection"
    "DAQ chopping"
...
Workflow:
  ...
  --from STAGE
      Start at: pythia, moretags, chop, starsim, mixer
  --stop-after STAGE
...
run_stage() {
    ...
    if (( RESUME )) && "$validate_fn" >/dev/null 2>&1; then
        if (( ! QUIET )); then
            printf '[%d/5] %-24s %sSKIPPED%s (already valid)\n' \
                "$((idx+1))" "$label" "${C_YELLOW}" "${C_RESET}"
        fi
        return 0
    fi
    ...
    if (( ! QUIET )); then
        ...
        printf '[%d/5] %-24s %sOK%s (%s)\n' \
            "$((idx+1))" "$label" "${C_GREEN}" "${C_RESET}" "$(format_seconds "$elapsed")"
    fi
}
...
clean_list_to_array "$DAQ_LIST" DAQ_ITEMS
clean_list_to_array "$MUDST_LIST" MUDST_ITEMS
...
mkdir -p \
    ...
    "$STARSIM_DIR" "$MIXER_DIR" "$LOG_DIR" "$DRIVER_DIR"
